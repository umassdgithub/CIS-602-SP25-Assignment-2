...
I have removed some part of the code and added ... and the points for each missing parts (there are 2 parts).
...
function displayPaintingLarge(clickedThumbImage) {
  ...
  let id = clickedThumbImage.dataset.id; // (10 points)

  // find that painting in array
  const painting = paintings.find(p => p.id == id); // (10 points)

  // display the found painting
  document.querySelector("#title").textContent = painting.title; // (10 points)
  document.querySelector("#artist").textContent = painting.artist; // (10 points)

  let image = document.createElement('img');
  image.src = "images/large/" + painting.id + ".jpg"; // (10 points)
